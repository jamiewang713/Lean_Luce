...
Authority: `fixed_points_shell_condition.tex`, not the older manuscript or the migration proposal. Implementation began only after creating `Luce/ShellMigrationContract.lean`. No generalized theorem is certified until the separate closed contract check is proved. The contract remains a proposition, not an axiom or a theorem assertion.
...
| `ass:fixed-endpoint`, lines 253–280 | Exact definitions in `EndpointShellDefinitions.lean`. Shells have `j ≥ 1`; labels use depth `n-k.val`, so the rate is literally `θ[n,n-m+1]`. Ratios inside `Real.log` are real. A finite minimum is taken only with a proof of nonemptiness. Empty shells have cost zero. The raw iterated limit is ENNReal `Tendsto (fun J => limsup (fun n => ∑' j, if J ≤ j then shellCost w n j else 0) atTop) atTop (𝓝 0)`. No normalization, buffered bound, tightness, or finite intensity is conjoined into it. |
