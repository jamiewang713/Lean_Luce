...
points was conjectured.  We prove this conjecture under minimal profile and
endpoint conditions.  The fixed points converge to an inhomogeneous Poisson
...
on the weight profile and a one-sided condition at the final labels.  We prove
...
The endpoint condition is not merely a device for the proof.  Its failure
reveals a second regime.  Very small weights ring late, while very large
weights ring early.  If a power-law singularity aligns one of these effects
with the corresponding endpoint labels, then every logarithmic distance
scale contributes a constant number of cycles.  The means are now of order
...
The only support condition concerns the final labels, which are the labels
that could become fixed at the final positions.  We rule out vanishing rates
there; no analogous bound is imposed in the interior.
...
Under Assumptions~\ref{ass:profile} and~\ref{ass:endpoint},
...
Under Assumptions~\ref{ass:profile} and~\ref{ass:endpoint}, for every fixed
$L$,
...
For $m=1,\ldots,M$, write $k_m=n-m+1$ and
$N_{-k}(t)=\sum_{j\ne k}\one\{E_j>t\}$.  The key deterministic observation is
...
Proposition~\ref{prop:endpoint-bound} uses no convergence of the profile, no
global lower bound, and no upper bound at all.  Only the mean-one normalization
and the lower bound on the labels that could be fixed in the tail enter the
proof.  Thus Assumption~\ref{ass:endpoint} can be weakened to any condition
that makes the right side of~\eqref{eq:tail-epsilon} tend to zero; the uniform
$\gamma$ is a transparent support-level formulation.
\end{remark}
...
\end{align*}
For $\alpha$ sufficiently close to one,
Proposition~\ref{prop:endpoint-bound} bounds the last expression by
$C(1-\alpha)^{\gamma/4}$.  Letting $\beta\uparrow1$ proves both
...
We next remove vertices with exceptional rates.  For $M>1$ and
$0<\delta<\gamma$, let
\[
...
\begin{proposition}\label{prop:exceptional-cycles}
Under Assumptions~\ref{ass:profile} and~\ref{ass:endpoint},
\begin{align}
 \lim_{M\to\infty}\limsup_n\E V_{n,L}(H_{n,M})&=0,
 \label{eq:high-cycle-truncation}\\
 \lim_{\delta\downarrow0}\limsup_n\E V_{n,L}(L_{n,\delta})&=0.
 \label{eq:low-cycle-truncation}
\end{align}
\end{proposition}
...
\end{equation}
This proves \eqref{eq:high-cycle-truncation}.

It remains to control low-rate vertices in cycles containing no high-rate
vertex.  Let $\beta=1-\eps_0$.  Since $\delta<\gamma$, every
$i\in L_{n,\delta}$ satisfies $i<\beta n$.  Lemma
...
\[
 ...
 \le K_{L,M}/n.
\]
A cycle meeting both exceptional sets contains at most $L$ low-rate vertices
per high-rate vertex.  Therefore
\[
 \E V_{n,L}(L_{n,\delta})
...
\eqref{eq:low-cycle-truncation}.
\end{proof}

After the exceptional vertices are removed, only the rates on the cycle
itself are bounded above and below; the background array remains arbitrary.

\begin{lemma}\label{lem:truncated-cycle-tail}
Fix $0<\delta<M<\infty$.  Let $C_{n,\ell}^{[\delta,M]}$ count cycles all of
whose vertex rates lie in $[\delta,M]$, and add a superscript $(\alpha)$ when
all their labels are at most $\alpha n$.  Then
\begin{equation}\label{eq:truncated-cycle-tail}
 \lim_{\alpha\uparrow1}\limsup_n
 \E\sum_{\ell=1}^L
 \left(C_{n,\ell}^{[\delta,M]}
 -(C_{n,\ell}^{[\delta,M]})^{(\alpha)}\right)=0.
\end{equation}
\end{lemma}

\begin{proof}
In the time-integral expansion of a cyclic product, every marked density is
at most $Me^{-\delta t}$.  At each time there are at most $2\ell+1$ possible
target windows.  If a proposed cycle meets $(\alpha n,n]$, one time exceeds
$T_{\lfloor\alpha n\rfloor-L}$.  Thus its complete cyclic sum is bounded by
\begin{equation}\label{eq:truncated-cyclic-sum}
 K_{L,\delta,M}e^{-\delta T_{\lfloor\alpha n\rfloor-L}}.
\end{equation}

To average this bound using only the normalization, take the at least $n/2$
background rates which are at most $2$.  Their clocks stochastically dominate
i.i.d. rate-$2$ exponentials.  By the time the $k$th full clock arrives, at
most $n-k$ clocks in this subfamily remain.  The standard beta moment for the
corresponding near-maximum exponential order statistic gives, for fixed
$a>0$ and $k=\lfloor\alpha n\rfloor-L$,
\begin{equation}\label{eq:late-order-moment}
 \E e^{-aT_k}
 \le K_{a,L}\bigl((1-\alpha)+L/n\bigr)^{a/2}.
\end{equation}
Apply this with $a=\delta$ in \eqref{eq:truncated-cyclic-sum}.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:short-cycles}]
For $\alpha<1$, let $C_{n,\ell}^{(\alpha)}$ count cycles all of whose labels
are at most $\alpha n$.  Fix nonnegative integers $m_1,\ldots,m_L$ and put
...
Proposition~\ref{prop:exceptional-cycles} and
Lemma~\ref{lem:truncated-cycle-tail} imply the full endpoint tightness
\begin{equation}\label{eq:cycle-tail-tightness}
 \lim_{\alpha\uparrow1}\limsup_n
 \E\sum_{\ell=1}^L
 \bigl(C_{n,\ell}-C_{n,\ell}^{(\alpha)}\bigr)=0.
\end{equation}
Indeed, first choose $M$ large and $\delta$ small so cycles containing an
exceptional-rate vertex have negligible expectation, and then apply
\eqref{eq:truncated-cycle-tail} to all remaining cycles.

If $\alpha<\beta<1$, first-moment convergence and
\eqref{eq:cycle-tail-tightness} show that
$\lambda_\ell(\beta)-\lambda_\ell(\alpha)$ tends uniformly to zero as
$\alpha\uparrow1$.  Hence the increasing $\lambda_\ell(\alpha)$ have finite
limits, necessarily the integrals in \eqref{eq:cycle-intensity}.  Markov's
inequality removes the rank cutoff in \eqref{eq:cycle-factorial-moments} and
...
and Proposition~\ref{prop:exceptional-cycles} overcome this obstruction by
swapping a finite insertion path into the background and separately charging
exceptional rates.  The finite-path lemma is distribution-free; the
truncation bounds for another clock family must come from suitable density
comparisons and endpoint envelopes.
