...
data = (root / "audit/section5-contract-and-roots.log").read_bytes()
...
for report in reports:
    names = {name.strip() for name in report.split("[", 1)[1][:-1].split(",")}
    assert names <= allowed, report
(root / "audit/section5-contract-and-roots-axioms.txt").write_text("\n".join(reports) + "\n", encoding="utf-8")
checks = {}
