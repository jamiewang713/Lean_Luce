...
/-- Choose an early fixed multiple of (n/h)^beta at which the actual
deleted survivor mean is at least 8h. No mean condition is assumed. -/
theorem PowerProfile.right_early_deleted_mean {f : ℝ → ℝ}
    {left : EndpointBehavior} {c beta eta : ℝ}
    (hp : PowerProfile f left (.power c beta eta)) (r : ℕ) :
    ∃ k H delta : ℝ, 0 < k ∧ k ≤ 1 ∧ 0 < H ∧ 0 < delta ∧ delta < 1 ∧
    ∀ (grid : SamplingGrid) (w : WeightArray), SampledRates grid w f →
    ∀ n h : ℕ, 0 < n → 0 < h → H ≤ (h : ℝ) → (h : ℝ)/(n : ℝ) < delta →
    ∀ removed : Finset (Fin n), removed.card ≤ r →
      8*(h : ℝ) ≤ (n : ℝ)*deletedH (w n) removed (k*((n : ℝ)/(h : ℝ))^beta) := by
  ...
  have hm : M ≤ (n : ℝ)*(k*((n : ℝ)/(h : ℝ))^beta)^(-(1/beta)) := by
    rw [hpow]
    have he : (n : ℝ)*((h : ℝ)/(u*(n : ℝ))) = (h : ℝ)/u := by
  ...
  rw [hpow] at hmean
  have he : (n : ℝ)*((A*((h : ℝ)/(u*(n : ℝ))))/2) = (A/(2*u))*(h : ℝ) := by
    field_simp [ne_of_gt hu, ne_of_gt hnR]
  change (n : ℝ)*((A*((h : ℝ)/(u*(n : ℝ))))/2) ≤ _ at hmean
  rw [he] at hmean
