...
/-- Quantitative inversion of a negative-power population error. The
...
theorem negative_power_inverse_estimate {A p q K : ℝ}
    (hA : 0 < A) (hp : 0 < p) (hq : 0 < q) (hK : 0 < K) :
    ∃ C : ℝ, 0 < C ∧ ∀ N x t : ℝ, 0 < N → 0 < x → 0 < t →
      |x-A*t^(-p)| ≤ K*(t^(-(p+q))+1/N) →
      K*t^(-q) ≤ A/4 → K/N ≤ x/4 →
      |t/(A/x)^(1/p)-1| ≤ C*(x^(q/p)+1/(N*x)) := by
  obtain ⟨L, hL, hlip⟩ := inverse_power_relative_error hp
  let B := (2*K/A)*(2/A)^(q/p)
  ...
  have htq : t^(-q) ≤ (2/A)^(q/p)*x^(q/p) := by
    ...
    exact hh
  have hscale : (t/(A/x)^(1/p))^(-p) = y/x := by
    have hT := Real.rpow_pos_of_pos (div_pos hA hx) (1/p)
    rw [Real.div_rpow ht.le hT.le, ← Real.rpow_mul (div_pos hA hx).le,
  ...
  have hi := hlip t ((A/x)^(1/p)) ht (Real.rpow_pos_of_pos (div_pos hA hx) _)
