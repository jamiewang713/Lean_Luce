...
/-- The collision estimate with a different bounded matrix for each cycle. -/
theorem collision_varying_matrices_pair_bound {s n A B L : ℕ} (D : ℕ)
    ...
    collisionPairSum (fun x : CollisionAssignment n A B side k => ∏ c, collisionCycleWeight (fun a b => F c a.val b.val) (k c) (x c)) D p q ≤
  ...
  have hw (c : Fin s) (x : Fin (k c+1) → CollisionCore n A B (side c)) : 0 ≤ w c x :=
    collisionCycleWeight_nonneg (fun a b : CollisionCore n A B (side c) => hF c a.val b.val) _ _
  ...
  · subst j
    ...
    have hblock : (∑ x, w i x*g x) ≤ Q^2*R := by
      have hh := collision_core_cycle_two_test (side i) (F i) (hF i) hC (hrow i) (htarget i) (hk i) p q hpq'
  ...
  · let g (x : Fin (k i+1) → CollisionCore n A B (side i))
    ...
    have hblock : (∑ x, ∑ y, w i x*w j y*g x y) ≤ Q^2*R := by
      have hh := collision_two_test_tensor (w i) (w j) (hw i) (hw j)
        ...
        (fun f hf => collision_core_cycle_test (side i) (F i) (hF i) hC (hrow i) (htarget i) (hk i) p f hf)
        (fun f hf => collision_core_cycle_test (side j) (F j) (hF j) hC (hrow j) (htarget j) (hk j) q f hf)
        (fun a b => if Nat.dist a.val.val b.val.val ≤ D then 1 else 0) (fun _ _ => by positivity)
