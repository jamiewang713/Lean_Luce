import Luce.Section4Endpoint
import Luce.Section4Fatou
import Luce.Section4Count
import Luce.Section4Compensator
import Luce.Section4IntensityEstimate
import Luce.Section4Intensity
import Luce.Section4EndpointWitness
import Luce.Section4FullIntensity
import Luce.Section4Approximation
import Luce.Section4CountLaw
import Luce.Section4LaplaceCountTightness
import Luce.Section4DiscreteTotalVariation
import Luce.Section4Poisson
import Luce.Section4TotalVariation
import Luce.Section4Theorem

/-! Checked entry point for all of Section 4 of `fixed_points.tex`.
The endpoint estimates, point-measure tail tightness, intensity finiteness,
full point-process limit and total-variation count limit are included.
See `docs/section4-status.md` for the source and proof audits.
-/
