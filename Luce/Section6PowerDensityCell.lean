...
/-- Compare a discrete depth with every point of its following unit cell.
Only the comparison envelope's decay constant is reduced. -/
theorem power_density_cell_bound {h x A d : ℝ} (hh : 1 ≤ h)
    (hx : h ≤ x) (hxh : x ≤ h+1) (hA : 0 < A) (hd : 0 < d) (p : ℝ) :
    A*h^(p-1)*Real.exp (-(d*A*h^p)) ≤
      (2 : ℝ)^|p-1| * (A*x^(p-1)*Real.exp (-((d/(2 : ℝ)^|p|)*A*x^p))) := by
  have hh0 : 0 < h := zero_lt_one.trans_le hh
  have hx0 : 0 < x := hh0.trans_le hx
  have hx2 : x ≤ 2*h := by linarith
  have hpref := (comparable_depth_rpow_bounds hh0 hx hx2 (p-1)).1
